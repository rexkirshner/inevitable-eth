...
Each node runs a local version of the EVM, which is then held perfectly in sync with every other copy of the EVM through a process called [Proof of Stake](https://inevitableeth.com/home/ethereum/network/consensus/pos) (PoS).
...
Consider a [block proposal](/ethereum/network/consensus/block-proposal). Every slot, a single validator, running on a single machine, controlled by a single person (or company) is chosen to have a full monopoly over what is included in the next block...
...
Good thing [Sreeram Kannan](https://twitter.com/sreeramkannan) has a solution: [EigenLayer](/ethereum/upgrades/middleware/eigenlayer).
