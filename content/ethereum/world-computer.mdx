...
Technically speaking, is a [Turing-complete](/concepts/turing-complete) distributed [state machine](/ethereum/evm/state-machine) - but don't worry about this specific classification.
...
A [block](/ethereum/blockchain/block) is made up of 3 key components: 
...
The method by which all these independent and untrusted computers keep their local copy of the EVM in sync is called a [consensus system](/ethereum/network/consensus).
...
[Deep Dive: Decentralized-Finance](/ethereum/world-computer/de-fi)

Ethereum offers all the same features and capabilities of a normal ([virtual](/concepts/virtual-machine)) computer, but with one critical difference: [internet native property rights](/ethereum/evm/property-rights).

From internet native property rights comes [internet native settlement](/ethereum/world-computer/internet-settlement). 
...
[Programmable Money](/ethereum/world-computer/de-fi/programmable-money). Decentralized Finance.
...
No one can tell you what the future holds, but the internet is [not the first time technology changed the way humans coordinate](/background/mass-comm/printing-press), and it will not be the last. 
...
The trilemma only makes sense if you think of Ethereum as a typical [computer](/background/mass-comm/computer), with a CPU and memory and the like.
...
[Rollups](/ethereum/upgrades/scaling/execution) will offer a spectrum of centralization/trust assumptions and performance trade-offs, but all will post a record back to Ethereum.

Each rollup is an independent blockchain that posts a (compressed) copy of every transaction it processes back to Ethereum.

As rollups improve execution, they will need to post an increasing amount transaction records; [Danksharding](/ethereum/upgrades/scaling/data) will VASTLY increase capacity for these records.
...
[Deep Dive: Accessing Ethereum](/ethereum/network/node/accessing-ethereum)
...
[Deep Dive: The Road to Stateless Ethereum](/ethereum/upgrades/statelessness)
...
[Deep Dive: The Portal Network](/ethereum/upgrades/light-clients)
...
[Deep Dive: Trustless Trust](/ethereum/world-computer/endgame/trustless-trust)
...
And [EigenLayer](/ethereum/upgrades/middleware/eigenlayer) will allow ETH to extend its untrusted-trust property onto ANY application that requires a centralized (or otherwise trusted) actor.
...
*   Ethereum: The Big Picture
    ...
    *   Source Material - [PDF](/twitter-pdf-2022-09-03_big_picture.pdf)
*   Ethereum: The Complete Story
    *   Source Material - [Twitter Link](https://twitter.com/logarithmicrex/status/1592309373732151296)
    *   Source Material - [PDF](/twitter-pdf-2022-11-15_the_complte_story.pdf)
