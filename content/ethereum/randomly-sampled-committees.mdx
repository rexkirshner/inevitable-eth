...
Tl;dr Ethereum will scale by [offloading execution to performance-optimized blockchains](/ethereum/upgrades/scaling/execution), while [settlement](/concepts/settlement) and economic security will remain with mainnet.

## Data Availability

![](/images/randomly-sampled-committees-2.jpeg)

[Deep Dive: Data Availability Scaling](/ethereum/upgrades/scaling/data/data-availability-bottleneck)
...
ALL nodes needs a copy of EVERY EVM [state change](/ethereum/evm/state-machine).
...
Using a [random seed](/ethereum/network/consensus/randao), we assign each validator to a committee arbitrarily. A malicious node operator has no way to control which committees his validators are assigned to.
