...
Each node runs a local version of the EVM, which is then held perfectly in sync with every other copy of the EVM through a process called [Proof of Stake](/ethereum/network/consensus/pos) (PoS).
...
[Deep Dive: Node](/ethereum/network/node)
...
DVT gives Ethereum tools it needs to continue decentralizing, further cementing its [credibly neutral](/concepts/credible-neutrality) position as the [settlement](/concepts/settlement) [layer of the internet](/ethereum/world-computer/internet-settlement).
...
Need an example? [One of the core technologies](/concepts/kzg-commitment) needed to enable [Danksharding](/ethereum/upgrades/scaling/data) requires advanced [elliptic curve cryptography](/concepts/elliptic-curve-cryptography) run on every block.
