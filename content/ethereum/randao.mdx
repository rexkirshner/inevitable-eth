...
Ethereum is the [World Computer](/ethereum/world-computer), a globally shared computing platform that exists between a network of 1,000s of [computers](/ethereum/node), each running a local version of the [Ethereum Virtual Machine (EVM)](/ethereum/evm). 
...
When a block proposer creates a new block, it adds in its contribution. [You can see the contribution in the block (randao\_reveal)](/ethereum/block). 
