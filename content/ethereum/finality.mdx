...
These nodes must stay perfectly in sync, processing the same [transactions](/ethereum/blockchain/transaction) and doing the same actions that every other node is doing.

The way Ethereum coordinates and stays in sync is through a process called [Proof of Stake](/ethereum/network/consensus/pos) (PoS).
...
[Deep Dive: Ethereum Time (Slots and Epochs)](/ethereum/network/consensus/time)
