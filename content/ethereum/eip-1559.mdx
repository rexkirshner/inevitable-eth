...
A [block](/ethereum/blockchain/block) contains a list of changes within the EVM ([transactions](/ethereum/blockchain/transaction)). Every node processes every new block, keeping all the EVMs in sync.

[Every 12 seconds](/ethereum/network/consensus/time) a new block is created, and so every node must be capable of processing every block (syncing their EVM) in less than 12 seconds.
