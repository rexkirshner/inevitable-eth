...
These nodes must stay perfectly in sync, processing the same [transactions](https://inevitableeth.com/home/ethereum/blockchain/transaction) and doing the same actions that every other node is doing.

The way Ethereum coordinates and stays in sync is through a process called [Proof of Stake](https://inevitableeth.com/home/ethereum/network/consensus/pos) (PoS).

## Ethereum Time

[Deep Dive: Time in Ethereum](/ethereum/network/consensus/time)

Before we continue, it's important to understand time on the World Computer.

*   [Blocks](/ethereum/blockchain/block) (bundles of transactions) execute atomically: all together or not at all
*   Every 12 seconds, Ethereum produces a block, filling a slot.
*   32 slots = 1 epoch

## Finality

[Deep Dive: Finality](/ethereum/network/consensus/finality)

Every 12 seconds, a randomly selected validator [proposes](/ethereum/network/consensus/block-proposal) a new block to the network.
...
Over the next 32 slots, each committee will [vote](/ethereum/network/consensus/attestation) on the block proposed in its corresponding slot.
