...
A plasma chain is a separate blockchain anchored to Ethereum but that executes transactions off-chain (with its own [consensus system](/ethereum/consensus)).
...
The plasma chain is its own blockchain with its own [virtual machine](/concepts/virtual-machine) and [state](/ethereum/state-machine). 
