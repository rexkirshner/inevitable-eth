...
[Deep Dive: Ethereum Scaling](/ethereum/upgrades/scaling/execution)

In order to scale without sacrificing decentralization, Ethereum is developing based on a rollup-centric roadmap.

Tl;dr [transactions](/ethereum/blockchain/transaction) will be moved off-chain to high performance (likely centralized) rollups, while [settlement](/concepts/settlement) will remain on Ethereum mainnet.

One of the more exciting applications of ZK-proofs are [ZK-rollups](/ethereum/upgrades/scaling/execution/zk-rollup), enabling layers on top of the World Computer that inherit the benefits Ethereum but are much more scalable.

This technology, with [additional mainnet upgrades](/ethereum/upgrades/scaling/data), will scale Ethereum to 100k+ txns/sec.
...
Once generated, the new [state](/ethereum/evm/state-machine), transactions and the ZK-proof are posted to a smart contract.
...
The goal of type 1 ZK-EVMs is to be able prove (execution-layer) [blocks](/ethereum/blockchain/block) in full, as they exist on the [blockchain](/ethereum/blockchain).
...
One of the particularly challenging data structures to create a ZK-proof for is a [Merkle Tree](/concepts/merkle-tree), a data structure Ethereum used to store a lot of the system critical information, including the [EVM state](/ethereum/evm/state-machine).
...
Source Material - [Twitter Link](https://twitter.com/SalomonCrypto/status/1570984357774655489)

Source Material - [PDF](/twitter-pdf-2022-09-16_zk_evm_taxonomy.pdf)
